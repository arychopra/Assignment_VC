...
for (i in unique(c(x_full$From, x_full$To))){
  
    
  
    dat <- x_full %>%
        left_join(select(df.emp, FullName, CurrentEmploymentTitle, CurrentEmploymentType), by = c("From"="FullName")) %>%
    ...
    g<-ggplot(dat, aes(y=To, x=Date.Time))+ 
        geom_point(position="jitter", 
                   size=3,alpha = 0.5, 
                   aes(text = sprintf("Text: %s<br>Date: %s<br>To_Title:%s<br>To_Dep:%s<br>", Subject2, Date.Date, To_title, To_dep))) +
        labs(y="",x="",title = paste(i,paste(unique(details), collapse = ',')))
