...
```{r betweeness, warning=F, message=F, echo=F, eval=F}
...
```{r eigenvalue, warning=F, message=F, echo=F, eval=F}
...
```{r closeness, eval=FALSE, warning=F, message=F, echo=F}
email_network4 <- graph_from_data_frame(d = links, 
                                        vertices = nodes_df, 
                                        directed = FALSE)
...
2. Feed into a for-loop to generate the conversation plot_ly object for each employee
...
for (i in unique(m$s)){
  ...
  temp_g <- m %>% 
              ...
              geom_point(aes(text = sprintf("Sub: %s<br>Date: %s<br>To: %s<br>To_Title: %s<br>To_Dep: %s<br>Time Sent: %s<br>", 
                                      Subject2, 
                                      Date.Date, 
                                      To,
                                      To_title, 
                                      To_dep,
                                      Date.Time), color=factor(Date.Date))) +
              labs(title=i) +
...
age_df <- df.emp %>% 
          ...
          mutate(hjust = ifelse(Approx_age >60, 1.05, -0.1)) %>%
...
age_df %>% 
    ...
    geom_point(aes(color = CurrentEmploymentType), size=3)  +
...
2. Two charts will be created, one for Kronos and the other for Tethys. The code line marks `--1` and `--2` will be changed to create the new charts.
