...
```{r, eval=F}
adjacencyData <- with(links, table(From, To))
...
for (i in sort(unique(c(x_full$From, x_full$To)))){
    ...
    g<-ggplot(dat, aes(y=To, x=Date.Time))+ 
        geom_point(position="dodge",
                   size=2,alpha = 0.5, shape=21,color="black", stroke=0.5, 
        ...
        labs(y="",x="Time",title = paste(i,paste(unique(details), collapse = ',')), group="", color="") +
...
ggplot(aes(y = FullName, x = MilitaryStartYear)) + 
  geom_point(size=3, color="#81b879") +
  geom_point(aes(MilitaryDischargeYear), color="#769ab5", size=3) +
  geom_segment(aes(x=MilitaryStartYear, xend=MilitaryDischargeYear, y=FullName,yend=FullName), size=0.8, alpha=0.7) +
  ...
  labs(x = "Year",y ="", title="Time Served in the Military")+
  theme(panel.background = element_rect(fill ="white"),
...
visNetwork(nodes = data_army$nodes, 
