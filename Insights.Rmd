...
### Characteristics of the News Sources
...
Possible members of the POK coud be in the Facilies or Security department, with the exception of Ruscella Mies Haber, who is in the admin department and assistant to the engineering group manager, Lidelse Dedos.
...
Isia Vann, who is Juliana Vann's brother and a confirmed member of the POK is approximately the same age as the other suspected members of the POK, except for Ruscella Mies Haber. Her age and middle name suggest that she might have been married to Valentine Mies. Any person with close links to Isia Vann is likely a member of the POK. 


```{r age, out.width=700}
knitr::include_graphics('images/5-Age.png')
```


We can see that, by the text groupings, most of the employees are within their department except for Linnea Bergen. She might also be a part of the extended network of POK. 

```{r, out.width=700}
knitr::include_graphics('images/5-TextNetwork.png')
```


```{r, out.width=700}
knitr::include_graphics('images/5-TextNetworkPOK.png')
```
...
knitr::include_graphics('images/5-3-5.png')
```

We can see that there is an overlap between the time served in the army. 

```{r army_kronos, out.width=700}
knitr::include_graphics('images/5-3-2.png')

```


```{r army_tethys, out.width=700}
...
knitr::include_graphics('images/5-3-7.png')
```


In addition to that, Isia Vann seems to be in a relationship with Rachel Pantanal, the authencity of the relationship from Isia's end could be doubted as she is in a crucial position, being the assistant to the CIO, Ada Campo-Corrente. However, despite Rachel being a citizen of Tethys, she nevertheless forwarded an email by the APA. 

This might indicate a few reasons, she is involved in the APA, they're planning to split the ransom amount or she believes in Isia's cause. 
...
Carmine Bodrogi was arrested in 2011 on the intent of possible distribution of MDMC, which is a drug trafficked by the APA. In 2014, Carmine Bodrogi is approximately 21 years old. She might be siblings with Loreto Bodrogi.
...
There are possible suggestion of links between the POK and the APA. This highly confirms that Hennie Osvaldo, Inga Ferro, Loreto Bodrogi, Minke Mies and Ruscella Mies Haber are the extended network of POK. 
...
These could be warnings about possible drug busts around town. 

```{r}
knitr:: include_graphics('images/Email-3-1.png')
```

The last names might such possible involvement in drug trafficking. They might be APA.  
...
Possibly Isia Vann was not in charge of the perimeter control for that day, hence it was not sent to him. But Loreto Bodrogi covers that. This raises suspicion and increases the likelihood that Loreto Bodrogi is indeed a member of the POK. 
