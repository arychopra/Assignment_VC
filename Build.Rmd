...
emails_grouped %>% 
      left_join(df.emp, by=c("From"="FullName")) %>%
      select(c(From, To, word, CurrentEmploymentType)) %>%
      rename(From_dep = CurrentEmploymentType) %>%
      left_join(df.emp, by=c("To"="FullName")) %>%
      select(c(From, To, word, From_dep, CurrentEmploymentType)) %>%
      rename(To_dep = CurrentEmploymentType)
...
node_s <- data.frame(name = unique(c(link_s$From, link_s$To)))
